...
# Constants
NETWORK_URL = "https://rpc.ankr.com/botanix_testnet"
CHAIN_ID = 3636
EXPLORER_URL = "https://testnet.botanixscan.io/tx/0x"
...
CONFIG = {
    ...
    "MAX_CONCURRENCY": 5,
    "MAX_RETRIES": 3,
    "MINIMUM_BALANCE": 0.000001  # BTC
}

# Từ vựng song ngữ
LANG = {
    'vi': {
        'title': '✨ GỬI GIAO DỊCH - BOTANIX TESTNET ✨',
        'info': 'ℹ Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
        ...
        'enter_amount': '✦ NHẬP SỐ LƯỢNG BTC',
        'amount_prompt': 'Số lượng BTC (mặc định 0.000001, tối đa 999): ',
        'amount_unit': 'BTC',
        ...
        'insufficient_balance': 'Số dư không đủ (cần ít nhất {required:.6f} BTC cho giao dịch)',
        ...
        'connect_success': '✅ Thành công: Đã kết nối mạng Botanix Testnet',
    ...
    'en': {
        'title': '✨ SEND TRANSACTION - BOTANIX TESTNET ✨',
        'info': 'ℹ Info',
        'found': 'Found',
        'wallets': 'wallets',
        ...
        'enter_amount': '✦ ENTER AMOUNT OF BTC',
        'amount_prompt': 'Amount of BTC (default 0.000001, max 999): ',
        'amount_unit': 'BTC',
        ...
        'insufficient_balance': 'Insufficient balance (need at least {required:.6f} BTC for transaction)',
        ...
        'connect_success': '✅ Success: Connected to Botanix Testnet',
    ...
    for attempt in range(CONFIG['MAX_RETRIES']):
        try:
            ...
            if balance < required_balance:
                print(f"{Fore.RED}  ✖ {LANG[language]['insufficient_balance'].format(required=required_balance)}: {balance:.6f} BTC{Style.RESET_ALL}")
                return False
...
async def process_wallet(index: int, profile_num: int, private_key: str, proxy: str, w3: Web3, choice: str, tx_count: int, amount: float, addresses: list, language: str):
    ...
    if eth_balance < CONFIG['MINIMUM_BALANCE']:
        print(f"{Fore.RED}  ✖ {LANG[language]['insufficient_balance'].format(required=CONFIG['MINIMUM_BALANCE'])}: {eth_balance:.6f} BTC{Style.RESET_ALL}")
        return 0
